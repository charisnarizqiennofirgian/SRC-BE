...
class RelinkSalesOrderToFixedProdukJadi extends Command
{
    ...
    {
        $this->info('Cari baris sales_order_details yang masih nyambung ke item Komponen lama...');

        $totalRelinked = 0;

        foreach ($this->originalCodes as $code) {
            $oldCode = $code . '-KOMP';
            $oldItem = Item::where('code', $oldCode)->first();

            if (!$oldItem) {
                $this->line("  - Kode lama '{$oldCode}' tidak ditemukan (sudah pernah dijalankan sebelumnya, atau app:fix-komponen-code-collision belum dijalankan). Dilewati.");
                continue;
            }
            ...
            $affectedDetails = DB::table('sales_order_details as sd')
                ...
                ->select('sd.id', 'so.so_number', 'sd.item_name', 'sd.quantity')
                ->get();

            if ($affectedDetails->isEmpty()) {
                $this->line("  - Tidak ada baris SO yang masih nyambung ke '{$oldCode}' (id={$oldItem->id}). Dilewati.");
                continue;
            }

            foreach ($affectedDetails as $detail) {
                // Cek dulu tidak ada delivery/invoice yang sudah pakai baris ini, demi keamanan
                $shipped = DB::table('sales_order_details')->where('id', $detail->id)->value('quantity_shipped');
                if ((float) $shipped > 0) {
