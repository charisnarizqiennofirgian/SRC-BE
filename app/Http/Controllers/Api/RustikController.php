...
class RustikController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        return DB::transaction(function () use ($data) {
            Log::info('=== RUSTIK START ===', ['po_id' => $data['ref_po_id']]);

            $sourceWarehouse = Warehouse::find($data['source_warehouse_id']);
            $targetWarehouse = Warehouse::where('code', 'RUSTIK')->first();

            if (!$targetWarehouse) {
                throw ValidationException::withMessages([
                    'warehouse' => ['Gudang Rustik tidak ditemukan.'],
                ]);
            }
            ...
            $runningNumber  = InventoryLog::where('transaction_type', 'RUSTIK')
                ->whereYear('date', now()->year)
                ->whereMonth('date', now()->month)
                ->where('direction', 'OUT')
                ->count() + 1;
            $documentNumber = 'RST-' . now()->format('Ym') . '-' . str_pad($runningNumber, 3, '0', STR_PAD_LEFT);
            ...
            foreach ($data['items'] as $index => $item) {
                ...
                InventoryLog::create([
                    ...
                    'transaction_type' => 'RUSTIK', 'reference_type' => 'ProductionOrder',
                    'reference_id' => $data['ref_po_id'],
                    'reference_number' => $documentNumber,
                    'notes' => "Masuk Rustik dari {$sourceName} ({$documentNumber}) - PO: {$poNumber}",
                    'user_id' => Auth::id(),
                ]);
                ...
                InventoryLog::create([
                    ...
                    'reference_id' => $data['ref_po_id'],
                    'reference_number' => $documentNumber,
                    'notes' => "Hasil Rustik masuk Gudang Rustik ({$documentNumber}) - PO: {$poNumber}",
                    'user_id' => Auth::id(),
                ]);
            }
            ...
            Log::info('=== RUSTIK END ===', ['doc' => $documentNumber]);

            return response()->json([
                'success' => true,
                'message' => "Proses Rustik berhasil ({$documentNumber})",
